...
select * from cars where year > 2015 and model = 'audi';
...
select count(model) as sumModel, model from cars group by model;
-- знайти марку машин кількість яких найбільше
select count(model) as sumModel, model from cars group by model order by sumModel desc limit 1;
